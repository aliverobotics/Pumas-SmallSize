...
globle void GetQueryFactSlot(
  ...
  DATA_OBJECT *result)
  {
   ...
   else if (FindSlot(theFact->whichDeftemplate,(symbolHashNode *)temp.value,&position) == NULL)
     {
      SlotExistError(theEnv,ValueToString(temp.value),"fact-set query");
      return;
     }
   ...
   if (val->type == DEFTEMPLATE_PTR)
     {
      ...
      head->template2 = (struct deftemplate *) val->value;
   ...
   if (val->type == SYMBOL)
     {
      ...
      IncrementDeftemplateBusyCount(theEnv,(void *) template2);
      head = get_struct(theEnv,query_template);
      head->template2 = template2;
      ...
      while (qlist->chain != NULL)
        {
         ...
         DecrementDeftemplateBusyCount(theEnv,(void *) tmp->template2);
   ...
   for (qptr = qchain ; qptr != NULL ; qptr = qptr->chain)
     {
      ...
      if (TestForFirstFactInTemplate(theEnv,qptr->template2,qchain,indx))
        { return(TRUE); }
   ...
   for (qptr = qchain ; qptr != NULL ; qptr = qptr->chain)
     {
      ...
      TestEntireTemplate(theEnv,qptr->template2,qchain,indx);
