...
// bind the socket to the address (only on RX_SOCKET mode)
   ...
   {    
		...
		//useless since is different 127.0.0.1 from real local ip in windows
		//GetIPAddress (hostname, addr);

		if (strcmp (hostname, "localhost")==0 || strcmp (hostname, "127.0.0.1")==0) {
			if (GettLocalIPAddress (addr)<0)
			return( MSOCKET_IMPOSSIBLE_TO_BIND );
		}
		else
			strcpy (addr, hostname);

		if ((sd=OpenSocketServer( addr, port ))<0)
			return( MSOCKET_IMPOSSIBLE_TO_BIND );
   }
#endif
