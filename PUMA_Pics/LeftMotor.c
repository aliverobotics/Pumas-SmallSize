...
//Slave definition
#use i2c(SLAVE, SDA=PIN_C4, SCL=PIN_C3, address=LEFT_MOTOR)
...
{														//	NOTHING -[slave addr](nothing)> CONTROL_READ -[addr](save addr value)> ADDRESS_READ -- [data] (writes data in buffer) -------> NOTHING
   byte incoming;								//																  										 						 			   \ 											   /	
...
void Motor(byte DIR) {			//Check motor's manual for further details
	...
	switch(DIR)
		...
		case 0x00 :// Deactivate
				...
				output_low(PIN_C0);
			   break;
	   case 0x01 :// Turns Clockwise	(Forward)
				output_high(PIN_C0);
				output_low(PIN_C1);
			   break;
...
void main ()
{
	...
	for ( i = 0; i < 0x10; i++)
		...
		buffer[1] = 0xff; 			/* PWM low byte */			//defautl PWM is at 0x1ff  [max]
		...
		actPWMDuty = byte2Long(buffer[1], buffer[2]);		//Sets default PWM duty		
